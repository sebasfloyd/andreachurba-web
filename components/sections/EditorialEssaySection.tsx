      ...
      >
        {Array.from({ length: 4 }).map((_, i) => (
          <span key={i} className="inline-flex items-center gap-12">
            <span className="font-serif italic text-ink text-[clamp(28px,4.5vw,68px)] leading-[1.1] tracking-tightmid">
...
export default function EditorialEssaySection() {
  ...
  return (
    <section className="bg-light text-ink py-24 md:py-32">
      <div className="container max-w-[1440px] mb-20">
      ...
      <div ref={containerRef} className="relative">
        <div className="container max-w-[1440px] mb-10">
          <motion.div
          ...
          >
            ...
            <h2 className="text-ink text-[clamp(28px,4vw,52px)] font-semibold tracking-tightest leading-[1.05]">
              Frases que repito porque las sostengo.
            </h2>
          </motion.div>
        </div>

        <motion.div style={{ y }} className="relative space-y-4 md:space-y-6">
        ...
        <div className="container max-w-[1440px] mt-12">
          ...
          >
            {citasReales.map((c, i) => (
              <span
                key={i}
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-full ${c.color} text-light text-[11px] font-bold uppercase tracking-widewide`}
              >
                {c.fuente}
              </span>
            ))}
